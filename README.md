...
2. ```npm install```<br>
**Note:** Because harp uses Ruby for sass and other feaure under he hood in background, you might see error logs in your screen just run this ```npm install grunt-harp``` with sudo before the next command)
3. ```bower install```
